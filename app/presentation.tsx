...
import { SystemProfiles, WorkGallery, ConnectionsScene, BoundaryScene } from './continuation';
...
function SlideContent({ step, phase, active, openExamples }: { step: number; phase: number; active: boolean; openExamples: (value: boolean) => void }) {
  if (step === 0) return <Opening active={active} />;
  ...
  if (step === 9) return <BioDemo active={active} />;
  return <ThankYou />;
}
