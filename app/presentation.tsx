...
import { SystemProfiles, WorkGallery, ConnectionsScene, ReviewScene, BoundaryScene, DiscoveryScene, BuildScene, Questions } from './continuation';
...
import { config, frames, prompts, sources, steps } from './content';
...
function SlideContent({ step, phase, active, openExamples }: { step: number; phase: number; active: boolean; openExamples: (value: boolean) => void }) {
  ...
  if (step === 6) return <ReviewScene phase={phase} active={active} openExamples={openExamples} />;
  if (step === 7) return <BoundaryScene active={active} />;
  if (step === 8) return <DiscoveryScene active={active} />;
  if (step === 9) return <BuildScene active={active} />;
