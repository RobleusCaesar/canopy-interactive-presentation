...
export const frames = steps.flatMap((_, step) => Array.from({ length: step === 0 ? 2 : step === 1 ? 4 : step === 2 ? 7 : step === 6 ? 5 : step === 8 ? 8 : 1 }, (_, phase) => ({ step, phase })));

export const answerPages = ['Chat', 'Agent introduction', 'Agent examples', 'Software'];

export const positionKey = 'canopy-presentation-v4';
export function restorePosition(current:string|null, previous:string|null, legacy:string|null) {
  const value=Number(current??previous??legacy);
  if(!Number.isInteger(value)||value<0)return 0;
  const oldPosition=previous!==null?value:value<=15?value:value===16?15:value===17?20:value<=19?21:29;
  const migrated=current!==null?value:oldPosition>=4?oldPosition+1:oldPosition;
